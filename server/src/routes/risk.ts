...
import { riskEngine, type RiskInput } from '../services/riskEngine.js'
 ...
 */
router.post('/simulate-disaster', (req: Request, res: Response) => {
  try {
    const { rainfall = 30, soilMoisture = 50, slope = 35, rainfallIncrease = 15, steps = 5 } = req.body

    const initialInput: RiskInput = {
      rainfall,
      soilMoisture,
      slope,
      elevation: 1500,
 ...
 */
router.get('/all-zones', (_req: Request, res: Response) => {
  try {
    const zones = [
      {
        id: 'east-sikkim',
