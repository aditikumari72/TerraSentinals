...
class RiskEngine {
    ...
    calculateRisk(input) {
        ...
        const riskScore = rainfallScore * weights.rainfall +
    ...
    calculateRainfallScore(rainfall) {
        ...
        if (rainfall < 20)
            return 0;
        if (rainfall < 50)
            return (rainfall - 20) * 1.2;
        if (rainfall < 100)
            return 36 + (rainfall - 50) * 0.48;
        if (rainfall < 150)
            return 60 + (rainfall - 100) * 0.5;
        return 85 + Math.min((rainfall - 150) * 0.3, 15);
    }
    calculateMoistureScore(moisture) {
        // Soil moisture risk increases with moisture content
        // 0% = 0%, 30% = 10%, 60% = 50%, 80% = 80%, 100% = 100%
        if (moisture < 30)
            return moisture * 0.33;
        if (moisture < 60)
            return 10 + (moisture - 30) * 1.33;
        if (moisture < 80)
            return 50 + (moisture - 60) * 1.5;
    ...
    calculateSlopeScore(slope) {
        ...
        if (slope < 15)
            return slope * 1.33;
        if (slope < 30)
            return 20 + (slope - 15) * 1.67;
        if (slope < 45)
            return 45 + (slope - 30) * 2;
        if (slope < 60)
            return 75 + (slope - 45) * 1.33;
    ...
    calculateElevationScore(elevation) {
        ...
        if (elevation < 500)
            return 5;
        if (elevation < 1000)
            return 5 + (elevation - 0) * 0.04;
        if (elevation < 1500)
            return 25 + (elevation - 1000) * 0.03;
        if (elevation < 2500)
            return 40 + (elevation - 1500) * 0.02;
        return 60 + Math.min((elevation - 2500) * 0.005, 15);
    }
    calculateProbability(riskScore, input) {
    ...
    generateRecommendation(riskLevel, _input) {
        switch (riskLevel) {
            case 'LOW':
     ...
     */
    simulateDisasterProgression(initialInput, rainfallIncrease, steps = 5) {
        const results = [];
